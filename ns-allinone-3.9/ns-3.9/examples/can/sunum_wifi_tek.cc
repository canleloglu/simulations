...
main (int argc, char *argv[])
{
  ...
  staInterfaces = address.Assign (staDevices);
	...
	serverApps.Add(echoServer.Install (wifiApNodes.Get(0)));
	echoServer.setIpv4Addr(serverApps.Get(0), apInterfaces.GetAddress(0));
	echoServer.setMode(serverApps.Get(0), "wifi");
	
	UdpEchoClientHelper echoClient (apInterfaces.GetAddress (0), 9);
	echoClient.SetAttribute ("MaxPackets", UintegerValue (2000));
	echoClient.SetAttribute ("Interval", TimeValue (Seconds (.05)));
	echoClient.SetAttribute ("PacketSize", UintegerValue (1024));

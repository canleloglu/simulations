...
MeshTest::InstallApplication ()
{
	...
	ofstream out1;
	out1.open("z_chart/meshClient.txt");
	out1.close();

	ofstream out2;
	out2.open("z_chart/meshServer.txt");
	out2.close();
	
	UdpEchoServerHelper echoServer (9);
	ApplicationContainer serverApps = echoServer.Install (nodes.Get (nAp));
	echoServer.setIpv4Addr(serverApps.Get(0), interfaces.GetAddress(nAp));
	...
	//{
		/*ostringstream oss;
		oss << "a_ap/" <<interfaces.GetAddress(i);
		...
		while(getline(input, line))
		{
			...
			UdpEchoClientHelper echoClient (interfaces.GetAddress (nAp), 9);
			echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
			echoClient.SetAttribute ("Interval", TimeValue (Seconds (m_packetInterval)));
			echoClient.SetAttribute ("PacketSize", UintegerValue (m_packetSize));
			ApplicationContainer clientApps = echoClient.Install (nodes.Get (i));
			echoClient.SetAddr(clientApps.Get(0), interfaces.GetAddress(i));
	...
	ifstream input;
	input.open("z_chart/wifiServer.txt");
	while(getline(input,line))
	{
		vector<string> vec = split(line, '$');
		string time = vec[vec.size()-1];
		...
		echoClient.SetAddr(clientApps.Get(0), interfaces.GetAddress(0));

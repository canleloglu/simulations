...
main (int argc, char *argv[])
{
    ...
    for (int i = 0; i < totalUserCount; i++) {
        ...
        if (luck == 0) {
            ...
            role = "worker";
        }else role = "nonworker";//dummy state
        wifiStaNodes.Get(i)->nodeRole = role;
        wifiStaNodes.Get(i)->clientID = i;
    }
        ...
        for(int staNum = apNum * g_userPerAp; staNum < apNum * g_userPerAp + g_userPerAp; staNum++)
        {
            ...
            {                
                ...
                if (active) 
                {
                    ...
                    onOffHelper.setUserRole(spokeApps.Get(0), wifiStaNodes.Get(staNum)->nodeRole);
                    onOffHelper.setClientID(spokeApps.Get(0), wifiStaNodes.Get(staNum)->clientID);
                    ...
                    if (r <= stayActiveProb) {
                        if ((int)tempsec % 10 == 0) {  
                            ...
                            ApplicationContainer hubApp2 = packetSinkHelper2.Install (apNode/*wifiApNodes.Get(apNum)*/);
                            packetSinkHelper2.setAddr(hubApp2.Get(0), apAddress /*apInterfaces.GetAddress(apNum)*/);
                            ...
                            ApplicationContainer spokeApps2;
                            AddressValue remoteAddress2 (InetSocketAddress (apAddress/*apInterfaces.GetAddress(apNum)*/, port2));
                            ...
                            spokeApps2.Add (onOffHelper2.Install (staNode));
                            onOffHelper2.setPacketId(spokeApps2.Get(0), packetID);
                            onOffHelper2.setAddr(spokeApps2.Get(0), staAddress);
                            onOffHelper2.setPath(spokeApps2.Get(0), folderName + "1srcWifi/");
                            onOffHelper2.setNetworkProtocol(spokeApps.Get(0), "UpdatePackets");
                            ...
                            spokeApps2.Start (Seconds (tempsec));
                            spokeApps2.Stop (Seconds (g_totalSimulationTime));
                        }
                        if ((int)tempsec % 60 == 0) {

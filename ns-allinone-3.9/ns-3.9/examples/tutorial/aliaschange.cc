...
{
	string folderName = "tcpExamples/ClientToTTP/";
	
	ofstream out;
	out.open("tcpExamples/ClientToTTP/srcWifi/10_1_3_1.txt");
	out.close();
	ofstream out2;
	out2.open("tcpExamples/ClientToTTP/destWifi/10_1_3_2.txt");
	out2.close();
	...
	double tmpTime = 1;
	for(int i = 1; tmpTime <= totalTime; i++)
	{
		
		double wifiDelayConstant = 0.001;
		double clientPuUsageDelay = 0.031;
		...
		clientApps.Add (onOffHelper.Install (wifiStaNode.Get(0)));
		onOffHelper.setAddr(clientApps.Get(0), staInterfaces.GetAddress(0));
		onOffHelper.setPacketId(clientApps.Get(0), i);
		onOffHelper.setPath(clientApps.Get(0), folderName + "srcWifi/");
		clientApps.Start(Seconds (tmpTime));
		...
		ApplicationContainer sApps = sinkHelper.Install (wifiStaNode.Get(0));
		sinkHelper.setAddr(sApps.Get(0), staInterfaces.GetAddress(0));
		...
		cApps.Add (onOff.Install (wifiApNode.Get(0)));
		onOff.setAddr(cApps.Get(0), apInterfaces.GetAddress(0));
		onOff.setPacketId(cApps.Get(0), i);
		cApps.Start(Seconds (tmpTime + wifiDelayConstant));
		...
		ApplicationContainer sApps2 = sinkHelper2.Install (wifiApNode.Get(0));
		sinkHelper2.setAddr(sApps2.Get(0), apInterfaces.GetAddress(0));
		sinkHelper2.setDirection(sApps2.Get(0), "gidis");
		sinkHelper2.setNetType(sApps2.Get(0), "wifi");
		sApps2.Start (Seconds (1.0));
		sApps2.Stop (Seconds (totalTime));
		
		AddressValue remAddress2 (InetSocketAddress (apInterfaces.GetAddress(0), prt2));		
		...
		cApps2.Add (onOff2.Install (wifiStaNode.Get(0)));
		onOff2.setAddr(cApps2.Get(0), staInterfaces.GetAddress(0));
		onOff2.setPacketId(cApps2.Get(0), i);
		cApps2.Start(Seconds (tmpTime + wifiDelayConstant + clientPuUsageDelay));

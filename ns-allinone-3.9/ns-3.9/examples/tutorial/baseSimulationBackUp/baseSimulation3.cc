...
main (int argc, char *argv[])
{
    ...
    for (int i = totalOtherEntities + 1; i <= totalOtherEntities + 3 ; i++) {
        ofstream out;
        string _out = folderName + "3srcCsma/10_1_3_" + itoa(i, 10) + ".txt";
        out.open(_out.c_str());
        out.close();
        ofstream out2;
        string _out2 = folderName + "3destCsma/10_1_3_" + itoa(i, 10) + ".txt";
  ...
  csmaInterfaces = address.Assign (csmaDevices);
	...
	string line;
	ifstream input;
    string _input = folderName + "2destMesh/10_1_3_21.txt";
    ...
    int clientIndex = totalOtherEntities;

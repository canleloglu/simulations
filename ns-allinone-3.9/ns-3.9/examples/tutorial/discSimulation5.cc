...
m_packetInterval (1),
m_packetSize (g_packetToClientSize),
m_nIfaces (1),
m_chan (true),
m_pcap (false),
...
MeshTest::InstallApplication ()
{
    ...
    for(int i = g_totalUserCount + 1; i <= g_totalUserCount + g_totalApCount + g_totalGwCount; i++)
    {
        ...
        ifstream input;
        string _input = folderName + "4destCsma/10_1_"+itoa(ip1, 10)+"_" + itoa(ip2, 10) + ".txt";
        ...
        while(getline(input, line))
        {
            ...
            serverApps.Add(packetSinkHelper.Install (nodes.Get(g_totalUserCount)));
            packetSinkHelper.setAddr(serverApps.Get(0), interfaces.GetAddress(g_totalUserCount));
            packetSinkHelper.setPath(serverApps.Get(0), folderName + "5destMesh/");
            ...
            if(netProtocol != "PacketTransfer" && netProtocol != "SeamlessMobility" && netProtocol != "Roaming"){            
                double arrangedTime = atof(time.c_str());
                AddressValue remoteAddress (InetSocketAddress (interfaces.GetAddress(g_totalUserCount), port));
                ...
                clientApps.Add (onOffHelper.Install (nodes.Get(g_totalUserCount+1)));
                onOffHelper.setPacketId(clientApps.Get(0), atoi(tmpPacketId.c_str()));
                onOffHelper.setAddr(clientApps.Get(0), interfaces.GetAddress(g_totalUserCount+1));
                onOffHelper.setPath(clientApps.Get(0), folderName + "5srcMesh/");
                ...
                clientApps.Start (Seconds (arrangedTime + g_rsaVerificationAPDelay));

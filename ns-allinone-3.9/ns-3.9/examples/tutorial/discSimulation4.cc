...
uint32_t indexOfNode(NodeContainer container, Ipv4InterfaceContainer interface, string ipAddr){
    Ipv4Address addr;
...
main (int argc, char *argv[])
{
    ...
    for (int i = totalOtherEntities + 1; i <= totalOtherEntities + totalCsmaUserCount ; i++) {
        ...
        ofstream out;
        string _out = folderName + "4srcCsma/10_1_"+itoa(ip1, 10)+"_" + itoa(ip2, 10) + ".txt";
        out.open(_out.c_str());
        out.close();
        ofstream out2;
        string _out2 = folderName + "4destCsma/10_1_"+itoa(ip1, 10)+"_" + itoa(ip2, 10) + ".txt";
  ...
  csmaInterfaces = address.Assign (csmaDevices);
    ...
    int inputIndexip1 = (totalOtherEntities + totalCsmaUserCount) / 256;
    int inputIndexip2 = (totalOtherEntities + totalCsmaUserCount) % 256;
	
	string line;
	ifstream input;
    string _input = folderName + "3destCsma/10_1_"+itoa(inputIndexip1, 10)+"_"+ itoa(inputIndexip2, 10) +".txt";
	...
	while(getline(input, line))
	{
        if(line.length() > 10){
            ...
            int serverIndex = indexOfNode(nodes, csmaInterfaces, orgGw);
            ...
            onOffHelper.SetAttribute ("PacketSize", UintegerValue(g_packetToClientSize));
            onOffHelper.SetAttribute ("MaxBytes", UintegerValue(g_packetToClientSize));
            ...
            clientApps.Start(Seconds (arrangedTime + 2 * g_rsaSigningOPDelay));

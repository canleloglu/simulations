...
{ 
	...
	*/
	...
	apInterfaces = address.Assign(apDevices); ///silinecek
	UdpEchoServerHelper echoServer(9); 
	ApplicationContainer servApps = echoServer.Install(wifiAPNodes.Get(2)); 
	servApps.Start(Seconds(1.0)); 
	servApps.Stop(Seconds(17.0)); 
	UdpEchoClientHelper echoClient(apInterfaces.GetAddress(2), 9); 
	ApplicationContainer clientApps = echoClient.Install(wifiStationNodes.Get(0)); 
	...
	oss << "/NodeList/" << wifiStationNodes.Get (0)->GetId() <<

...
MeshTest::InstallApplication ()
{
	...
	ofstream out;
	out.open("tcpExamples/ClientToTTP/srcMesh/10_1_3_2.txt");
	out.close();
	ofstream out2;
	out2.open("tcpExamples/ClientToTTP/destMesh/10_1_3_21.txt");
	out2.close();
	...
	packetSinkHelper.setPath(serverApps.Get(0), folderName + "destMesh/");

	ifstream input;
	input.open("tcpExamples/ClientToTTP/destWifi/10_1_3_2.txt");
	int dummy = 10000;
	while(getline(input, line))
	{
		...
		onOffHelper.setPath(clientApps.Get(0), folderName + "srcMesh/");
		...
		if(m_randomTraffic)
		{
			//adding random traffic to system
			int dummySize = 256;
			if(arrangedTime >= 30)
				dummySize = 512;
			if(arrangedTime >= 45)
			dummySize = 256;
			UniformVariable ilkYari (0,10);
			UniformVariable ikinciYari (11,19);
			UniformVariable dummyDelay (0,1);

			int ilk = ilkYari.GetInteger(0,10);
			int ikinci = ikinciYari.GetInteger(0,10);
			double dDelay = dummyDelay.GetValue();
			...
			ApplicationContainer sApps = sinkHelper.Install (nodes.Get(ikinci));
			sinkHelper.setAddr(sApps.Get(0), interfaces.GetAddress(ikinci));
			sApps.Start (Seconds (1.0));
			sApps.Stop (Seconds (m_totalTime));
			
			AddressValue remAddress (InetSocketAddress (interfaces.GetAddress(ikinci), prt));		
			onOff.SetAttribute ("Remote", remAddress);
			onOff.SetAttribute ("PacketSize", UintegerValue(256));
			onOff.SetAttribute ("MaxBytes", UintegerValue(dummySize));
			ApplicationContainer cApps;
			cApps.Add (onOff.Install (nodes.Get(ilk)));
			onOff.setAddr(cApps.Get(0), interfaces.GetAddress(ilk));
			onOff.setPacketId(cApps.Get(0), dummy);
			cApps.Start(Seconds (arrangedTime + dDelay));
			cApps.Stop(Seconds (m_totalTime));
			dummy++;
		}	 
		//added random traffic to the system

...
main (int argc, char *argv[])
{
    ...
    for(int i = totalUserCount + 1; i <= totalUserCount + totalAccessPointCount; i++)
    {
        ...
        while (getline(in, line)) {
            ...
            uint32_t clientIndex = indexOfNode(orgClient, wifiStaNodes, staInterfaces);
            uint32_t apIndex = indexOfNode(orgAp, wifiApNodes, apInterfaces);
            
            ApplicationContainer sApps;
            sApps.Add(sHelper.Install (wifiStaNodes.Get(clientIndex)));
            ...
            cApps.Add (onOff.Install (wifiApNodes.Get(apIndex)));

...
main (int argc, char *argv[])
{
  ...
  staInterfaces = address.Assign (staDevices);
	...
	out1.open("z_chart/wifiClient.txt");
	out2.open("z_chart/wifiServer.txt");
	out1.close(); out2.close();
	
	serverApps.Add(echoServer.Install(wifiApNodes.Get(nAp-1)));
	echoServer.setIpv4Addr(serverApps.Get(0), apInterfaces.GetAddress(nAp-1));
	UdpEchoClientHelper echoClient (apInterfaces.GetAddress (nAp-1), 9);
	echoClient.SetAttribute ("MaxPackets", UintegerValue (50000));
	echoClient.SetAttribute ("Interval", TimeValue (Seconds (.1)));
	echoClient.SetAttribute ("PacketSize", UintegerValue (1024));
	clientApps.Add(echoClient.Install(wifiStaNodes.Get(99)));
	echoClient.SetAddr(clientApps.Get(0), staInterfaces.GetAddress(99));	

...
main (int argc, char *argv[])
{
	...
	if (enableTracing == true)
	{
		...
		//wifiPhy.EnablePcapAll("laan");
		
		// Let's do a pcap trace on the application source and sink, ifIndex 0
		// Csma captures in non-promiscuous mode
		
#if 0
		csma.EnablePcap ("mw_csma", appSource->GetId (), 0, false);
#else
		csma.EnablePcapAll ("mw_csma2", false);
#endif
		/*wifiPhy.EnablePcap ("mw_wifi", appSink->GetId (), 0);
		wifiPhy.EnablePcap ("mw_wifi", 9, 2);
		wifiPhy.EnablePcap ("mw_wifi", 9, 0);*/
		//wifiPhy.EnablePcap("mw_wifi",appSource);

...
Packet::CopyData(std::ostream *os, uint32_t size) const
{
  return m_buffer.CopyData (os, size);
	...
	void Packet::SetUid (uint64_t val)
	{
		m_metadata.SetUid (val);
	}
    
    std::string Packet::GetProtocol (void)
    {
        return m_metadata.GetProtocol();
    }
    
    void Packet::SetProtocol (std::string val)
    {
        m_metadata.SetProtocol(val);
    }
    
    string Packet::GetOrgClient(void) const{
        return m_metadata.GetOrgClient();
    }
    
    string Packet::GetOrgAp(void) const{
        return m_metadata.GetOrgAp();
    }
    
    string Packet::GetOrgGw(void) const{
        return m_metadata.GetOrgGw();
    }
    
    string Packet::GetOrgOp(void) const{
        return m_metadata.GetOrgOp();
    }
    
    void Packet::SetOrgClient(string s){
        m_metadata.SetOrgClient(s);
    }
    
    void Packet::SetOrgAp(string s){
        m_metadata.SetOrgAp(s);
    }
    
    void Packet::SetOrgGw(string s){
        m_metadata.SetOrgGw(s);
    }
    
    void Packet::SetOrgOp(string s){
        m_metadata.SetOrgOp(s);
    }

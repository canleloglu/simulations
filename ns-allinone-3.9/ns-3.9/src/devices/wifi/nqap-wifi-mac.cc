...
{	
	...
	//NS_LOG_UNCOND("received packet with id: " << packet->GetUid());
          ...
          if (to == GetAddress ()) 
            {
              NS_LOG_DEBUG ("frame for me from = "<<from);
				...
				/*fstream filestr;
				filestr.open ("access.txt", fstream::in | fstream::out | fstream::app);
				
				std::ostringstream oss;
				oss<<GetAddress()<<"$"<<from<<"$"<<to<<"$"<<packet<<"$"<<Simulator::Now().GetSeconds();
				filestr<<oss.str()<<std::endl;
				filestr.close();
				*/
				//ekleme sonu
